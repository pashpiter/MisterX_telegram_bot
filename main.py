...
async def first_message(message: types.Message) -> None:
    ...
    await send_text(
        ...
        'ребусы, раскрывать шифры и в конце получить кодовую фразу, состоящую '
        'из трех слов.'
    )
    await send_text(
        message, 'Итак, давай приступим. На данный момент у нас уже есть '
        'некоторые перехваченные данные, которые могут пригодится, они '
        'находятся у тебя в конверте: цифровой дешифратор, таблица Виженера, '
        'шифр с фигурами. Чтобы начать охоту за _Мистером Х_ следуй во *Двор '
