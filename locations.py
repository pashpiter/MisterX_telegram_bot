...
async def location_two(message: types.Message) -> None:
    ...
    await send_text(
        ...
        'ключа для шифра Виженера: *"Датчик"*\n\nДальше следуй во двор по '
        'адресу: _Малый проспект П.С. 1Б_'
    )
    await send_photo(
...
async def location_three(message: types.Message) -> None:
    ...
    await send_text(
        ...
        '25 10 4\n⭐ 34 17 35 23 35\n😎 19 21 20 15 21 23 10 21 13 20 32*\n\n'
    )
    await send_text(message, 'И вот ещё:')
...
async def location_five(message: types.Message) -> None:
    ...
    await send_photo(
        ...
        '%BE%D1%8F.jpg')
    await send_text(message, 'Буквы зашифрованного слова: *ПБЩЬДЧ*')
    await send_text(
...
async def location_nine(message: types.Message) -> None:
    ...
    logging.info(f'User {message.chat.id}, {message.chat.username} '
                 'location 9')
    await send_text(
        message, 'Это Санкт-Петербургский государственный электротехнический '
        'университет «ЛЭТИ» имени В. И. Ульянова (Ленина), студенты называют '
        'его Петербургским Хогвартсом из-за его архитектуры, башен и больших '
        'окон.\n7 мая 1895 года в стенах университета Александр Попов выступил'
        ' с докладом, в котором описал способ передачаи сигналов на расстояние'
        ' в 64 метра. А именно, Попов показал как производить передачу '
        'элементов азбуки Морзе с помощью радиосвязи. Что впоследствие '
        'привело к развитию новых средств связи.\nОдним из известных '
        'выпускников университета является *Алферов Жорес Иванович* '
        '(ученый-физик) и *Дмитрий Нагиев* также является выпускником '
        'электротехнического университета.', hide_kb
    )
